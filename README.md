...
> on an Android device, but it is still **experimental and slow** â€” performance work is ongoing.
...
- `android/` â€” a complete Gradle app using Kotlin for the UI and the NDK for the game runtime:
  - `MainActivity` â€” Storage Access Framework disc picker; imports and verifies your `.wbfs`/`.iso`
    into the app's private data directory, then enables launch.
  - `GameActivity` â€” fullscreen immersive surface, multitouch dispatcher, and accelerometer sensor
    feed into the native engine.
  - `app/src/main/cpp/` â€” JNI bridge (`libmkw_android.so`), Android touch input adapter,
    disc extraction bridge, and the Android render-loop / Vulkan wiring.
- `THIRD-PARTY-NOTICES.md` â€” full third-party license ledger and mandatory attributions.
...
The game logic, translation pipeline, and rendering core remain exactly the upstream engine â€”
...
- **Performance** â€” internal-resolution / pipeline tuning so full races run at acceptable speed.
...
prebuilt `libmkw_base_shared.a` archive (~1.8 GB, not tracked) â€” everyone builds these locally, and
...
### Step 1 â€” Install the toolchain

**Option A â€” Android Studio (easiest)**

1. Install [Android Studio](https://developer.android.com/studio).
2. In **SDK Manager â†’ SDK Platforms**, install **Android API 35**.
3. In **SDK Manager â†’ SDK Tools**, install **NDK; 28.2.13676358**, **CMake 4.1.2**, and
   **Android SDK Platform-Tools**.
4. Install a **JDK 17** and the **.NET 8 SDK**.

**Option B â€” Command line (no Android Studio)**
...
#### Option B â€” Command Line (One-shot bootstrap)
...
4. **Translate** â€” run the .NET translator to generate the C++ shard sources (`generated\`) and
   `shards.cmake`. *Only runs if these are missing.*
5. **Build shards** â€” compile all translated C++ into `libmkw_base_shared.a` (~1.8 GB, the long step).
6. **Build the APK** â€” `gradlew assembleRelease` (or `assembleDebug`).
7. **Install** â€” because you passed `-Install`, push the APK to your connected phone.
...
1. Tap **select discâ€¦** and pick your `RMCP01.wbfs` or `.iso`.
...
* **`Android NDK not found`** â€” `build-shards.bat` can't locate the NDK. Install
  `ndk;28.2.13676358` and re-run the bootstrap's Detect step, or set `ANDROID_NDK_HOME`.
* **`.NET 8 SDK required`** â€” the translator needs .NET 8 (only on a fresh clone's first build).
* **`Game disc files are required`** â€” put `main.dol` + `StaticR.rel` in `Assets\` or pass
  `-DiscSource`.
* **`ccache` errors** â€” it is optional and auto-detected; without it the build simply skips it.
* **First build is slow** â€” compiling ~29,000 translated functions into a 1.8 GB static archive is
...
> download site should not be trusted â€” you would have no idea what it contains.
...
- **WiiCompiled** â€” [patchzyy/Wiicompiled](https://github.com/patchzyy/Wiicompiled)
  - The entire static-recompilation engine: translator, runtime, Aurora/Dawn rendering, audio,
    and disc loading. **GPL-3.0.**
- **KartPad** â€” [chrissotraidis/kartpad](https://github.com/chrissotraidis/kartpad)
...
The complete ledger â€” every component, its license, its copyright holder, and the required
notices â€” is in **[`THIRD-PARTY-NOTICES.md`](THIRD-PARTY-NOTICES.md)**.
