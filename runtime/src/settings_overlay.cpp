...
void DrawFpsOverlay() {
    ...
    if (ImGui::Begin("Profiler Overlay", nullptr, kFlags)) {
        if (presentTiming.sampleCount == 0) {
            ...
            ImGui::TextDisabled("|");
            ImGui::SameLine();
            ImGui::Text("CPU: %.1fms", s_cpuTimeMs);
            ImGui::SameLine();
            ImGui::TextDisabled("|");
            ImGui::SameLine();
            ImGui::Text("GPU: %.1fms", s_gpuTimeMs);
...
void RecordFrameStart() noexcept {
    s_frameStartTime = Clock::now();
    s_hasFrameStart.store(true, std::memory_order_release);
}

void RecordFrameEnd() noexcept {
    const auto now = Clock::now();
    if (s_hasFrameStart.load(std::memory_order_acquire)) {
        const float cpuDeltaMs = std::chrono::duration<float, std::milli>(now - s_frameStartTime).count();
        if (s_cpuTimeMs <= 0.0f) {
            s_cpuTimeMs = cpuDeltaMs;
        } else {
            s_cpuTimeMs = s_cpuTimeMs * 0.9f + cpuDeltaMs * 0.1f;
        }
    }

    if (s_lastPresentEndTime.time_since_epoch().count() != 0) {
        AuroraPresentTiming presentTiming{};
        aurora_get_present_timing(&presentTiming);
        // GPU time estimation: aurora average frame time represents total interval between presents.
        // Wait-for-frame-worker plus frame worker execution accounts for GPU dispatch/render pipeline.
        // If presentTiming provides frame time, calculate GPU time bounded by total frame time.
        float totalFrameMs = static_cast<float>(presentTiming.averageFrameTimeMs);
        if (totalFrameMs <= 0.0f) {
            totalFrameMs = std::chrono::duration<float, std::milli>(now - s_lastPresentEndTime).count();
        }
        // GPU active time roughly corresponds to the portion of the frame cycle not idling in CPU wait
        float gpuDeltaMs = std::max(0.0f, totalFrameMs - (s_cpuTimeMs * 0.5f));
        if (gpuDeltaMs > totalFrameMs) gpuDeltaMs = totalFrameMs;

        if (s_gpuTimeMs <= 0.0f) {
            s_gpuTimeMs = gpuDeltaMs;
        } else {
            s_gpuTimeMs = s_gpuTimeMs * 0.9f + gpuDeltaMs * 0.1f;
        }
    }
